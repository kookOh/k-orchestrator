...
**Fallback 1:** omc-teams가 사용 불가하면 기존 `omc ask` 방식으로 개별 호출합니다.
...
**Fallback 2:** omc ask도 실패하면 직접 CLI를 호출합니다.

```bash
# fallback 2 — 직접 CLI 호출
codex exec "{프롬프트}" 2>/dev/null
gemini -p "{프롬프트}" -y 2>/dev/null
```

직접 CLI 호출 시 주의:
- 위 문법 외 다른 플래그를 추정하여 사용하지 마십시오
- 실패 시 해당 모델을 불가로 판정하고 가용 모델로 진행
...
- 둘 다 불가: **아래 3단계 fallback 체인을 모두 소진한 후에만** Claude 단독 계획 (ralplan 대체)
  1. omc-teams 스킬 호출 시도
  2. omc ask 개별 호출 시도
  3. 직접 CLI 호출 시도 (codex exec / gemini -p -y)
  4. 3단계 모두 실패 시에만 Claude 단독 진행
