...
8. Obsidian vault + Git + QMD 연동까지 구현한다 (QMD CLI 미설치 시 안내만)
...
## VAULT_DIR 자동 감지 및 설정
`VAULT_DIR` 환경변수가 설정되지 않은 경우, 구축 전에 다음 순서로 vault를 감지하십시오:

### 자동 감지 순서
1. `find ~ -maxdepth 4 -name ".obsidian" -type d` 실행하여 Obsidian vault 후보 탐색
2. 후보가 1개면 자동 선택, 2개 이상이면 사용자에게 선택 요청
3. 후보가 0개면 사용자에게 vault 경로를 직접 입력받거나 `~/Documents/Obsidian Vault/`에 새로 생성할지 확인

### 글로벌 설정 적용
vault 경로가 결정되면 `~/.claude/settings.json`의 `env.VAULT_DIR`에 기록하십시오.
이렇게 하면 모든 프로젝트에서 hook이 자동 동작합니다.
프로젝트별로 다른 vault를 사용하려면 프로젝트 `.claude/settings.json`의 `env.VAULT_DIR`로 오버라이드합니다.

## 실제 구축 규칙
`VAULT_DIR`가 자동 감지 또는 사용자 입력으로 결정된 후 구축을 수행한다.
...
### 6. QMD 컬렉션 등록 (선택적)
`qmd` CLI가 설치되어 있는 경우 현재 프로젝트의 문서를 QMD에 등록하십시오:

```bash
# 프로젝트 docs
qmd collection add "$PROJECT_ROOT/docs" --name "<project-slug>-docs" --mask "**/*.md"
# 프로젝트 tasks
qmd collection add "$PROJECT_ROOT/tasks" --name "<project-slug>-tasks" --mask "**/*.md"
# vault 세션 (글로벌, 이미 등록되어 있으면 스킵)
qmd collection add "$VAULT_DIR/sessions" --name vault-sessions --mask "**/*.md"
qmd collection add "$VAULT_DIR/projects" --name vault-projects --mask "**/*.md"
```

등록 후 `qmd update && qmd embed` 실행하여 인덱스와 벡터를 생성합니다.
`qmd` CLI가 없으면 QMD 단계를 스킵하고 안내만 출력합니다:
"QMD를 설치하면 시맨틱 검색이 가능합니다: https://github.com/nicobailey/qmd"

## 검증 요구사항
- `VAULT_DIR` 없을 때 자동 감지 시도 → 실패 시 사용자 입력 요청 → 그래도 없으면 non-fatal 종료
- 디렉토리 구조 재실행 시 중복/파괴 없음
- source of truth 우선순위 유지
- QMD 컬렉션 이미 존재 시 재등록하지 않음
