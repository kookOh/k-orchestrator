...
- `.claude/commands/k-orchestrator/` — 14개 command
- `.claude/skills/k-orchestrator/` — 3개 skill (2 policy + 1 감지)
...
## skills 설치 확인
install.sh는 skills를 `.claude/skills/k-orchestrator/`에 자동 설치합니다.
...
## 주의사항
- OMC가 제공하는 agent 역할명(`planner/architect/executor/verifier` 등)을 `.claude/agents/`에 두지 않는다
